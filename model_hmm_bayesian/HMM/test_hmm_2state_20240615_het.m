...
%%
%2 states
[seq,NLLout,Outpurparams,Startpoint]=gethmmstate2state(grin2a);
%params: 
%logLikelihood: -6.6813e3

% 3 states
[~,NLLout3,Outpurparams3,Startpoint3]=gethmmstate3state(grin2a);
%logLikelihood: -6.6635e3
%
%4 states
[seq,NLLout4,Outpurparams4,Startpoint4]=gethmmstate4state(grin2a);

%%
numtrials=0;

for i=1:length(grin2a)
...
BIC2=4*log(numtrials)+2*mode(NLLout); %1.3400e4
BIC3=9*log(numtrials)+2*mode(NLLout3);  %1.3410e4 <<
BIC4=16*log(numtrials)+2*mode(NLLout4);  %1.3208e4

%% AIC
% 2*k-2ln(L)

AIC2=4*2+2*mode(NLLout); % 1.3371e4
AIC3=9*2+2*mode(NLLout3);  % 1.3345e4 
AIC4=16*2+2*mode(NLLout4); % 1.3092e4
